...
if (isset($_GET['billStatus'])) {
    ...
    if ($billStatus==1) {
        $scriptAlert=' alert("Pay successsfully!")';
    }
    else {
        $scriptAlert=' alert("Pay unsuccesssfully!")';
    }
}
if (!isset($_SESSION['fullname']))  //kiểm tra đăng nhập
{
    ...
    if (isset($_GET['vegeId'])) 
    {
        ...
        if (check($object,$_SESSION['listVegeId'])) //kiểm tra mã sản phẩm vừa mua đã có trong giỏ hàng chưa
        {
            
        }else {
            array_push($_SESSION['listVegeId'],$object);//nếu chưa thì thêm vào giỏ hàng
        }
    }
...
$html='';
foreach ($_SESSION['listVegeId'] as $key=> $item) //xuất thông tin giỏ hàng với từng ID sản phẩm vừa thêm vào
{
        ...
        <table class="table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <th></th>
                    <th></th>
                    <th>Total:<?php echo $totalAmount?></th>
                    <th>Total: <?php echo $total;//hiển thị thông tin giỏ hàng ?></th>
                </tr>
            </tbody>
        </table>
